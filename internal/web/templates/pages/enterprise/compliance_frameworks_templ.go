...
func ComplianceFrameworks(data ComplianceFrameworksData) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "\" x-init=\"init()\" class=\"space-y-6\"><!-- Header --><div class=\"flex items-center justify-between\"><div><h1 class=\"text-2xl font-display font-bold text-white\">Compliance Frameworks</h1><p class=\"text-gray-400 mt-1\">SOC 2, HIPAA, PCI-DSS compliance management and assessment tracking</p></div><div class=\"flex items-center gap-2\"><button @click=\"seedFrameworks()\" :disabled=\"seeding\" class=\"btn-secondary flex items-center gap-2\"><i aria-hidden=\"true\" class=\"fas fa-database\" :class=\"seeding && 'animate-spin'\"></i> <span x-text=\"seeding ? 'Seeding...' : 'Seed Default Frameworks'\"></span></button></div></div><!-- Loading State --><div x-show=\"loading\" class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl text-primary-400 mb-4\"></i><p class=\"text-gray-400\">Loading compliance frameworks...</p></div><!-- Error State --><div x-show=\"error && !loading\" x-cloak class=\"card border-l-4 border-l-red-500 p-4\"><div class=\"flex items-center gap-3\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-circle text-red-400\"></i><div><h3 class=\"text-sm font-medium text-red-400\">Failed to load frameworks</h3><p class=\"text-xs text-gray-400 mt-0.5\" x-text=\"error\"></p></div><button @click=\"fetchFrameworks()\" class=\"ml-auto btn-secondary text-sm\">Retry</button></div></div><!-- Main Content --><div x-show=\"!loading && !error\" x-cloak class=\"space-y-6\"><!-- Tabs --><div class=\"flex gap-1 border-b border-dark-600\"><button @click=\"tab = 'frameworks'\" :class=\"tab === 'frameworks' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"tab-underline\"><i aria-hidden=\"true\" class=\"fas fa-layer-group mr-2\"></i>Frameworks <span class=\"ml-1 px-1.5 py-0.5 text-xs bg-dark-600 rounded\" x-text=\"frameworks.length\"></span></button> <button @click=\"tab = 'assessments'; if (allAssessments.length === 0 && !loadingAssessments) fetchAllAssessments()\" :class=\"tab === 'assessments' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"tab-underline\"><i aria-hidden=\"true\" class=\"fas fa-clipboard-check mr-2\"></i>Assessments <span class=\"ml-1 px-1.5 py-0.5 text-xs bg-dark-600 rounded\" x-text=\"allAssessments.length\"></span></button> <button @click=\"tab = 'evidence'; if (evidenceList.length === 0 && selectedAssessmentForEvidence) fetchEvidence(selectedAssessmentForEvidence)\" :class=\"tab === 'evidence' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"tab-underline\"><i aria-hidden=\"true\" class=\"fas fa-file-alt mr-2\"></i>Evidence <span class=\"ml-1 px-1.5 py-0.5 text-xs bg-dark-600 rounded\" x-text=\"evidenceList.length\"></span></button></div><!-- ==================== FRAMEWORKS TAB ==================== --><div x-show=\"tab === 'frameworks'\" x-cloak class=\"space-y-6\"><!-- Framework Cards --><div x-show=\"frameworks.length > 0\" class=\"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4\"><template x-for=\"fw in frameworks\" :key=\"fw.id\"><div class=\"card p-5 hover:border-primary-500/50 transition-colors\"><div class=\"flex items-start justify-between mb-4\"><div class=\"flex items-center gap-3\"><div class=\"w-10 h-10 rounded-lg flex items-center justify-center\" :class=\"frameworkIconBg(fw.name)\"><i aria-hidden=\"true\" class=\"fas\" :class=\"frameworkIcon(fw.name)\"></i></div><div><h3 class=\"font-medium text-white\" x-text=\"fw.display_name || fw.name\"></h3><p class=\"text-xs text-gray-500\" x-text=\"'v' + (fw.version || '1.0')\"></p></div></div><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"fw.is_enabled ? 'bg-green-500/20 text-green-400' : 'bg-gray-500/20 text-gray-400'\" x-text=\"fw.is_enabled ? 'Enabled' : 'Disabled'\"></span></div><p class=\"text-sm text-gray-400 mb-4 line-clamp-2\" x-text=\"fw.description || 'No description'\"></p><!-- Compliance Score Gauge --><div class=\"mb-4\"><div class=\"flex items-center justify-between mb-1\"><span class=\"text-xs text-gray-400\">Compliance Score</span> <span class=\"text-sm font-bold\" :class=\"scoreColor(fw._status ? fw._status.compliance_score : null)\" x-text=\"fw._status && fw._status.compliance_score != null ? Math.round(fw._status.compliance_score) + '%' : '--'\"></span></div><div class=\"w-full h-2 bg-dark-700 rounded-full overflow-hidden\"><div class=\"h-full rounded-full transition-all duration-500\" :class=\"scoreBarColor(fw._status ? fw._status.compliance_score : null)\" :style=\"'width: ' + (fw._status && fw._status.compliance_score != null ? fw._status.compliance_score : 0) + '%'\"></div></div></div><!-- Control Counts --><div x-show=\"fw._status\" class=\"grid grid-cols-4 gap-2 mb-4 text-center\"><div><p class=\"text-sm font-bold text-green-400\" x-text=\"fw._status ? fw._status.implemented || 0 : 0\"></p><p class=\"text-[10px] text-gray-500\">Done</p></div><div><p class=\"text-sm font-bold text-yellow-400\" x-text=\"fw._status ? fw._status.in_progress || 0 : 0\"></p><p class=\"text-[10px] text-gray-500\">In Prog</p></div><div><p class=\"text-sm font-bold text-gray-400\" x-text=\"fw._status ? fw._status.not_started || 0 : 0\"></p><p class=\"text-[10px] text-gray-500\">Not Started</p></div><div><p class=\"text-sm font-bold text-blue-400\" x-text=\"fw._status ? fw._status.not_applicable || 0 : 0\"></p><p class=\"text-[10px] text-gray-500\">N/A</p></div></div><!-- Actions --><div class=\"flex items-center gap-2 pt-3 border-t border-dark-700\"><button @click=\"openFrameworkDetail(fw)\" class=\"text-sm text-primary-400 hover:text-primary-300 transition-colors flex items-center gap-1 flex-1\"><i aria-hidden=\"true\" class=\"fas fa-list text-xs\"></i> Controls</button> <button @click=\"runAssessment(fw)\" :disabled=\"fw._assessing\" class=\"text-sm text-green-400 hover:text-green-300 transition-colors flex items-center gap-1 flex-1\"><i aria-hidden=\"true\" class=\"fas fa-play text-xs\" :class=\"fw._assessing && 'animate-spin'\"></i> <span x-text=\"fw._assessing ? 'Running...' : 'Assess'\"></span></button> <button @click=\"openAssessmentHistory(fw)\" class=\"text-sm text-gray-400 hover:text-gray-300 transition-colors flex items-center gap-1 flex-1\"><i aria-hidden=\"true\" class=\"fas fa-history text-xs\"></i> History</button></div></div></template></div><!-- Empty State --><div x-show=\"frameworks.length === 0\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-balance-scale text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">No Compliance Frameworks</h3><p class=\"text-gray-400 mb-4\">Seed default frameworks (SOC 2, HIPAA, PCI-DSS) to start compliance tracking</p><button @click=\"seedFrameworks()\" :disabled=\"seeding\" class=\"btn-primary\"><i aria-hidden=\"true\" class=\"fas fa-database mr-2\"></i>Seed Default Frameworks</button></div></div><!-- ==================== ASSESSMENTS TAB ==================== --><div x-show=\"tab === 'assessments'\" x-cloak class=\"space-y-4\"><!-- Assessments Loading --><div x-show=\"loadingAssessments\" class=\"card p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">Loading assessments...</p></div><!-- Assessments Table --><div x-show=\"!loadingAssessments && allAssessments.length > 0\" x-cloak class=\"card overflow-hidden\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Assessment</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Framework</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Status</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Score</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Controls</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Date</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Actions</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"a in allAssessments\" :key=\"a.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><div class=\"font-medium text-white\" x-text=\"a.name || ('Assessment #' + a.id.substring(0, 8))\"></div></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-300\" x-text=\"frameworkNameById(a.framework_id)\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"assessmentStatusClass(a.status)\" x-text=\"a.status\"></span></td><td class=\"px-4 py-3\"><span class=\"text-sm font-bold\" :class=\"scoreColor(a.score)\" x-text=\"a.score != null ? Math.round(a.score) + '%' : '--'\"></span></td><td class=\"px-4 py-3 text-sm text-gray-400\"><span class=\"text-green-400\" x-text=\"a.passed_controls || 0\"></span> <span class=\"text-gray-600\">/</span> <span class=\"text-red-400\" x-text=\"a.failed_controls || 0\"></span> <span class=\"text-gray-600\">/</span> <span x-text=\"a.total_controls || 0\"></span></td><td class=\"px-4 py-3 text-sm text-gray-400 whitespace-nowrap\" x-text=\"formatTimestamp(a.completed_at || a.started_at)\"></td><td class=\"px-4 py-3\"><div class=\"flex items-center gap-1\"><button @click=\"viewAssessmentDetail(a)\" class=\"p-1.5 text-gray-400 hover:text-primary-400 transition-colors\" title=\"View details\"><i aria-hidden=\"true\" class=\"fas fa-eye text-sm\"></i></button> <button @click=\"downloadReport(a.id, 'json')\" class=\"p-1.5 text-gray-400 hover:text-blue-400 transition-colors\" title=\"Download JSON report\"><i aria-hidden=\"true\" class=\"fas fa-file-code text-sm\"></i></button> <button @click=\"downloadReport(a.id, 'html')\" class=\"p-1.5 text-gray-400 hover:text-green-400 transition-colors\" title=\"Download HTML report\"><i aria-hidden=\"true\" class=\"fas fa-file-alt text-sm\"></i></button> <button @click=\"selectedAssessmentForEvidence = a.id; tab = 'evidence'; fetchEvidence(a.id)\" class=\"p-1.5 text-gray-400 hover:text-yellow-400 transition-colors\" title=\"View evidence\"><i aria-hidden=\"true\" class=\"fas fa-folder-open text-sm\"></i></button></div></td></tr></template></tbody></table></div><!-- Assessments Empty State --><div x-show=\"!loadingAssessments && allAssessments.length === 0\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-clipboard-check text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">No Assessments</h3><p class=\"text-gray-400\">Run an assessment from the Frameworks tab to evaluate your compliance posture</p></div></div><!-- ==================== EVIDENCE TAB ==================== --><div x-show=\"tab === 'evidence'\" x-cloak class=\"space-y-4\"><!-- Assessment Selector --><div class=\"flex items-center gap-3\"><div><select x-model=\"selectedAssessmentForEvidence\" @change=\"fetchEvidence(selectedAssessmentForEvidence)\" class=\"input text-sm\"><option value=\"\">Select an assessment...</option><template x-for=\"a in allAssessments\" :key=\"a.id\"><option :value=\"a.id\" x-text=\"(a.name || 'Assessment') + ' (' + formatTimestamp(a.completed_at || a.started_at) + ')'\"></option></template></select></div><button x-show=\"selectedAssessmentForEvidence\" @click=\"showAddEvidenceModal = true\" class=\"btn-primary flex items-center gap-2 text-sm\"><i aria-hidden=\"true\" class=\"fas fa-plus\"></i> Add Evidence</button></div><!-- Evidence Loading --><div x-show=\"loadingEvidence\" class=\"card p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">Loading evidence...</p></div><!-- Evidence Table --><div x-show=\"!loadingEvidence && evidenceList.length > 0\" x-cloak class=\"card overflow-hidden\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Title</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Control</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Type</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Status</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Collected</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"ev in evidenceList\" :key=\"ev.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><div class=\"font-medium text-white\" x-text=\"ev.title\"></div><div class=\"text-xs text-gray-500 mt-0.5 max-w-[300px] truncate\" x-text=\"ev.description\"></div></td><td class=\"px-4 py-3 text-sm text-gray-300 font-mono\" x-text=\"ev.control_id || '-'\"></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs bg-dark-600 text-gray-300 rounded\" x-text=\"ev.evidence_type || '-'\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"evidenceStatusClass(ev.status)\" x-text=\"ev.status || '-'\"></span></td><td class=\"px-4 py-3 text-sm text-gray-400 whitespace-nowrap\" x-text=\"formatTimestamp(ev.collected_at)\"></td></tr></template></tbody></table></div><!-- Evidence Empty State --><div x-show=\"!loadingEvidence && evidenceList.length === 0 && selectedAssessmentForEvidence\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-file-alt text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">No Evidence</h3><p class=\"text-gray-400 mb-4\">No evidence has been collected for this assessment yet</p><button @click=\"showAddEvidenceModal = true\" class=\"btn-primary\"><i aria-hidden=\"true\" class=\"fas fa-plus mr-2\"></i>Add Evidence</button></div><div x-show=\"!selectedAssessmentForEvidence\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-arrow-up text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">Select an Assessment</h3><p class=\"text-gray-400\">Choose an assessment from the dropdown to view or add evidence</p></div></div></div><!-- ==================== FRAMEWORK DETAIL MODAL (Controls) ==================== --><div x-show=\"showControlsModal\" x-cloak class=\"fixed inset-0 z-50 flex items-center justify-center bg-black/60\"><div class=\"bg-dark-800 rounded-xl shadow-xl border border-dark-600 w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col\"><div class=\"flex items-center justify-between p-6 border-b border-dark-600 shrink-0\"><div><h2 class=\"text-lg font-display font-bold text-white\" x-text=\"(selectedFramework ? (selectedFramework.display_name || selectedFramework.name) : '') + ' Controls'\"></h2><p class=\"text-sm text-gray-400 mt-0.5\" x-text=\"controls.length + ' controls'\"></p></div><button @click=\"showControlsModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-6 overflow-y-auto flex-1 space-y-4\"><!-- Controls Loading --><div x-show=\"loadingControls\" class=\"p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">Loading controls...</p></div><!-- Controls Filters --><div x-show=\"!loadingControls && controls.length > 0\" class=\"flex items-center gap-3\"><select x-model=\"controlStatusFilter\" class=\"input text-sm\"><option value=\"\">All Statuses</option> <option value=\"not_started\">Not Started</option> <option value=\"in_progress\">In Progress</option> <option value=\"implemented\">Implemented</option> <option value=\"not_applicable\">Not Applicable</option></select> <select x-model=\"controlSeverityFilter\" class=\"input text-sm\"><option value=\"\">All Severities</option> <option value=\"critical\">Critical</option> <option value=\"high\">High</option> <option value=\"medium\">Medium</option> <option value=\"low\">Low</option></select> <span class=\"text-sm text-gray-400\" x-text=\"filteredControls().length + ' controls'\"></span></div><!-- Controls Table --><div x-show=\"!loadingControls && controls.length > 0\" x-cloak class=\"overflow-hidden rounded-lg border border-dark-700\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600 bg-dark-900/50\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Control ID</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Title</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Category</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Severity</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Status</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"ctrl in filteredControls()\" :key=\"ctrl.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3 text-sm text-primary-400 font-mono\" x-text=\"ctrl.control_id\"></td><td class=\"px-4 py-3\"><div class=\"font-medium text-white text-sm\" x-text=\"ctrl.title\"></div><div class=\"text-xs text-gray-500 mt-0.5 max-w-[300px] truncate\" x-text=\"ctrl.description\"></div></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs bg-dark-600 text-gray-300 rounded\" x-text=\"ctrl.category || '-'\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"severityClass(ctrl.severity)\" x-text=\"ctrl.severity\"></span></td><td class=\"px-4 py-3\"><select :value=\"ctrl.implementation_status\" @change=\"updateControlStatus(ctrl.id, $event.target.value)\" class=\"input text-xs py-1 px-2\"><option value=\"not_started\">Not Started</option> <option value=\"in_progress\">In Progress</option> <option value=\"implemented\">Implemented</option> <option value=\"not_applicable\">Not Applicable</option></select></td></tr></template></tbody></table></div><!-- Controls Empty --><div x-show=\"!loadingControls && controls.length === 0\" x-cloak class=\"p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-list text-2xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">No controls found for this framework</p></div></div></div></div><!-- ==================== ASSESSMENT HISTORY MODAL ==================== --><div x-show=\"showHistoryModal\" x-cloak class=\"fixed inset-0 z-50 flex items-center justify-center bg-black/60\"><div class=\"bg-dark-800 rounded-xl shadow-xl border border-dark-600 w-full max-w-3xl mx-4 max-h-[90vh] flex flex-col\"><div class=\"flex items-center justify-between p-6 border-b border-dark-600 shrink-0\"><div><h2 class=\"text-lg font-display font-bold text-white\" x-text=\"(selectedFramework ? (selectedFramework.display_name || selectedFramework.name) : '') + ' Assessment History'\"></h2><p class=\"text-sm text-gray-400 mt-0.5\" x-text=\"historyAssessments.length + ' assessments'\"></p></div><button @click=\"showHistoryModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-6 overflow-y-auto flex-1\"><!-- History Loading --><div x-show=\"loadingHistory\" class=\"p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">Loading assessment history...</p></div><!-- History Table --><div x-show=\"!loadingHistory && historyAssessments.length > 0\" x-cloak class=\"overflow-hidden rounded-lg border border-dark-700\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600 bg-dark-900/50\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Name</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Score</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Passed / Failed</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Date</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Report</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"a in historyAssessments\" :key=\"a.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><div class=\"text-sm text-white\" x-text=\"a.name || 'Assessment'\"></div><div class=\"text-xs text-gray-500 font-mono\" x-text=\"a.id.substring(0, 8)\"></div></td><td class=\"px-4 py-3\"><span class=\"text-sm font-bold\" :class=\"scoreColor(a.score)\" x-text=\"a.score != null ? Math.round(a.score) + '%' : '--'\"></span></td><td class=\"px-4 py-3 text-sm\"><span class=\"text-green-400\" x-text=\"a.passed_controls || 0\"></span> <span class=\"text-gray-600\">/ </span> <span class=\"text-red-400\" x-text=\"a.failed_controls || 0\"></span></td><td class=\"px-4 py-3 text-sm text-gray-400 whitespace-nowrap\" x-text=\"formatTimestamp(a.completed_at || a.started_at)\"></td><td class=\"px-4 py-3\"><div class=\"flex items-center gap-1\"><button @click=\"downloadReport(a.id, 'json')\" class=\"p-1.5 text-gray-400 hover:text-blue-400 transition-colors\" title=\"JSON report\"><i aria-hidden=\"true\" class=\"fas fa-file-code text-sm\"></i></button> <button @click=\"downloadReport(a.id, 'html')\" class=\"p-1.5 text-gray-400 hover:text-green-400 transition-colors\" title=\"HTML report\"><i aria-hidden=\"true\" class=\"fas fa-file-alt text-sm\"></i></button></div></td></tr></template></tbody></table></div><!-- History Empty --><div x-show=\"!loadingHistory && historyAssessments.length === 0\" x-cloak class=\"p-8 text-center\"><i aria-hidden=\"true\" class=\"fas fa-history text-2xl text-gray-400 mb-2\"></i><p class=\"text-sm text-gray-400\">No assessments have been run for this framework yet</p></div></div></div></div><!-- ==================== ASSESSMENT DETAIL MODAL ==================== --><div x-show=\"showDetailModal\" x-cloak class=\"fixed inset-0 z-50 flex items-center justify-center bg-black/60\"><div class=\"bg-dark-800 rounded-xl shadow-xl border border-dark-600 w-full max-w-4xl mx-4 max-h-[90vh] flex flex-col\"><div class=\"flex items-center justify-between p-6 border-b border-dark-600 shrink-0\"><div><h2 class=\"text-lg font-display font-bold text-white\" x-text=\"detailAssessment ? (detailAssessment.name || 'Assessment Detail') : 'Assessment Detail'\"></h2><p class=\"text-sm text-gray-400 mt-0.5\" x-text=\"'Framework: ' + frameworkNameById(detailAssessment ? detailAssessment.framework_id : null)\"></p></div><button @click=\"showDetailModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-6 overflow-y-auto flex-1 space-y-6\"><!-- Score Ring and Summary --><div x-show=\"detailAssessment\" class=\"flex items-center gap-8\"><div class=\"relative w-28 h-28 shrink-0\"><svg class=\"w-28 h-28 -rotate-90\" viewBox=\"0 0 120 120\"><circle cx=\"60\" cy=\"60\" r=\"52\" fill=\"none\" stroke=\"#1e293b\" stroke-width=\"10\"></circle> <circle cx=\"60\" cy=\"60\" r=\"52\" fill=\"none\" :stroke=\"scoreRingStroke(detailAssessment ? detailAssessment.score : null)\" stroke-width=\"10\" stroke-linecap=\"round\" :stroke-dasharray=\"2 * Math.PI * 52\" :stroke-dashoffset=\"2 * Math.PI * 52 * (1 - (detailAssessment ? (detailAssessment.score || 0) : 0) / 100)\"></circle></svg><div class=\"absolute inset-0 flex items-center justify-center\"><span class=\"text-2xl font-bold text-white\" x-text=\"detailAssessment && detailAssessment.score != null ? Math.round(detailAssessment.score) + '%' : '--'\"></span></div></div><div class=\"grid grid-cols-2 gap-4\"><div class=\"card p-3 text-center\"><p class=\"text-lg font-bold text-white\" x-text=\"detailAssessment ? detailAssessment.total_controls || 0 : 0\"></p><p class=\"text-xs text-gray-400\">Total</p></div><div class=\"card p-3 text-center\"><p class=\"text-lg font-bold text-green-400\" x-text=\"detailAssessment ? detailAssessment.passed_controls || 0 : 0\"></p><p class=\"text-xs text-gray-400\">Passed</p></div><div class=\"card p-3 text-center\"><p class=\"text-lg font-bold text-red-400\" x-text=\"detailAssessment ? detailAssessment.failed_controls || 0 : 0\"></p><p class=\"text-xs text-gray-400\">Failed</p></div><div class=\"card p-3 text-center\"><p class=\"text-lg font-bold text-blue-400\" x-text=\"detailAssessment ? detailAssessment.na_controls || 0 : 0\"></p><p class=\"text-xs text-gray-400\">N/A</p></div></div></div><!-- Control Results (collapsible) --><div x-show=\"detailAssessment && detailAssessment.results && detailAssessment.results.length > 0\"><h3 class=\"text-sm font-medium text-gray-300 mb-3\">Control Results</h3><div class=\"space-y-1\"><template x-for=\"(result, idx) in (detailAssessment ? detailAssessment.results || [] : [])\" :key=\"idx\"><div class=\"border border-dark-700 rounded-lg overflow-hidden\"><button @click=\"result._open = !result._open\" class=\"w-full flex items-center justify-between px-4 py-2.5 hover:bg-dark-700/50 transition-colors\"><div class=\"flex items-center gap-3\"><i class=\"fas fa-circle text-[8px]\" :class=\"result.passed ? 'text-green-400' : (result.status === 'not_applicable' ? 'text-blue-400' : 'text-red-400')\"></i> <span class=\"text-sm text-white font-mono\" x-text=\"result.control_id || ('Control ' + (idx + 1))\"></span> <span class=\"text-sm text-gray-400\" x-text=\"result.title || ''\"></span></div><i aria-hidden=\"true\" class=\"fas text-gray-500 text-xs\" :class=\"result._open ? 'fa-chevron-up' : 'fa-chevron-down'\"></i></button><div x-show=\"result._open\" x-cloak class=\"px-4 py-3 border-t border-dark-700 bg-dark-900/50\"><div class=\"grid grid-cols-2 gap-4 text-sm\"><div><p class=\"text-gray-500 text-xs mb-0.5\">Status</p><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"result.passed ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'\" x-text=\"result.passed ? 'Passed' : 'Failed'\"></span></div><div><p class=\"text-gray-500 text-xs mb-0.5\">Severity</p><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"severityClass(result.severity)\" x-text=\"result.severity || '-'\"></span></div></div><div x-show=\"result.message\" class=\"mt-3\"><p class=\"text-gray-500 text-xs mb-0.5\">Details</p><p class=\"text-sm text-gray-300\" x-text=\"result.message\"></p></div><div x-show=\"result.remediation\" class=\"mt-3\"><p class=\"text-gray-500 text-xs mb-0.5\">Remediation</p><p class=\"text-sm text-gray-300\" x-text=\"result.remediation\"></p></div></div></div></template></div></div></div></div></div><!-- ==================== ADD EVIDENCE MODAL ==================== --><div x-show=\"showAddEvidenceModal\" x-cloak class=\"fixed inset-0 z-50 flex items-center justify-center bg-black/60\"><div class=\"bg-dark-800 rounded-xl shadow-xl border border-dark-600 w-full max-w-lg mx-4\"><div class=\"flex items-center justify-between p-6 border-b border-dark-600\"><h2 class=\"text-lg font-display font-bold text-white\">Add Evidence</h2><button @click=\"showAddEvidenceModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-6 space-y-4\"><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Title</label> <input type=\"text\" x-model=\"newEvidence.title\" class=\"input w-full\" placeholder=\"Evidence title\" aria-label=\"Title\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Control ID</label> <input type=\"text\" x-model=\"newEvidence.control_id\" class=\"input w-full font-mono\" placeholder=\"e.g. SOC2-CC6.1\" aria-label=\"Control ID\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Evidence Type</label> <select x-model=\"newEvidence.evidence_type\" class=\"input w-full\"><option value=\"manual\">Manual</option> <option value=\"automated\">Automated</option> <option value=\"screenshot\">Screenshot</option> <option value=\"log\">Log</option> <option value=\"document\">Document</option></select></div><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Description</label> <textarea x-model=\"newEvidence.description\" rows=\"2\" class=\"input w-full\" placeholder=\"Describe the evidence\"></textarea></div><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Data (JSON)</label> <textarea x-model=\"newEvidence.data\" rows=\"3\" class=\"input w-full font-mono text-sm\" placeholder='{\"key\": \"value\"}'></textarea></div><div class=\"flex justify-end gap-3 pt-4 border-t border-dark-600\"><button @click=\"showAddEvidenceModal = false\" class=\"btn-secondary\">Cancel</button> <button @click=\"createEvidence()\" :disabled=\"!newEvidence.title.trim() || addingEvidence\" class=\"btn-primary disabled:opacity-50\"><i aria-hidden=\"true\" class=\"fas fa-plus mr-2\" :class=\"addingEvidence && 'animate-spin'\"></i> <span x-text=\"addingEvidence ? 'Adding...' : 'Add Evidence'\"></span></button></div></div></div></div></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
	...
	return `{
		...
		async runAssessment(fw) {
			...
			try {
				...
				if (window.usulnet) { window.usulnet.toast('Assessment complete — Score: ' + (result.score != null ? Math.round(result.score) + '%' : 'N/A'), 'success'); }
