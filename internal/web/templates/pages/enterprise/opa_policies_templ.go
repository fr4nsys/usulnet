...
func OPAPolicies(data OPAPoliciesData) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "\" x-init=\"init()\" class=\"space-y-6\"><!-- Header --><div class=\"flex items-center justify-between\"><div><h1 class=\"text-2xl font-display font-bold text-white\">OPA Policies</h1><p class=\"text-gray-400 mt-1\">Open Policy Agent rule-based policy engine for container compliance</p></div><div class=\"flex items-center gap-2\"><button @click=\"seedPolicies()\" :disabled=\"seeding\" class=\"btn-secondary flex items-center gap-2\"><i aria-hidden=\"true\" class=\"fas fa-database\" :class=\"seeding && 'animate-spin'\"></i> <span x-text=\"seeding ? 'Seeding...' : 'Seed Default Policies'\"></span></button></div></div><!-- Loading State --><div x-show=\"loading\" class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl text-primary-400 mb-4\"></i><p class=\"text-gray-400\">Loading policies...</p></div><!-- Error State --><div x-show=\"error && !loading\" x-cloak class=\"card border-l-4 border-l-red-500 p-4\"><div class=\"flex items-center gap-3\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-circle text-red-400\"></i><div><h3 class=\"text-sm font-medium text-red-400\">Failed to load policies</h3><p class=\"text-xs text-gray-400 mt-0.5\" x-text=\"error\"></p></div><button @click=\"fetchPolicies()\" class=\"ml-auto btn-secondary text-sm\">Retry</button></div></div><!-- Stats --><div x-show=\"!loading && !error\" x-cloak class=\"grid grid-cols-2 md:grid-cols-4 gap-4\"><div class=\"card p-3 text-center\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt text-primary-400 text-lg mb-1\"></i><p class=\"text-xl font-bold text-white\" x-text=\"policies.length\"></p><p class=\"text-xs text-gray-400\">Total Policies</p></div><div class=\"card p-3 text-center\"><i aria-hidden=\"true\" class=\"fas fa-check-circle text-green-400 text-lg mb-1\"></i><p class=\"text-xl font-bold text-white\" x-text=\"policies.filter(p => p.is_enabled).length\"></p><p class=\"text-xs text-gray-400\">Enabled</p></div><div class=\"card p-3 text-center\"><i aria-hidden=\"true\" class=\"fas fa-pause-circle text-gray-400 text-lg mb-1\"></i><p class=\"text-xl font-bold text-white\" x-text=\"policies.filter(p => !p.is_enabled).length\"></p><p class=\"text-xs text-gray-400\">Disabled</p></div><div class=\"card p-3 text-center\"><i aria-hidden=\"true\" class=\"fas fa-tags text-blue-400 text-lg mb-1\"></i><p class=\"text-xl font-bold text-white\" x-text=\"[...new Set(policies.map(p => p.category))].length\"></p><p class=\"text-xs text-gray-400\">Categories</p></div></div><!-- Filters --><div x-show=\"!loading && !error && policies.length > 0\" x-cloak class=\"flex items-center gap-3\"><div><select x-model=\"categoryFilter\" class=\"input text-sm\"><option value=\"\">All Categories</option><template x-for=\"cat in categories\" :key=\"cat\"><option :value=\"cat\" x-text=\"cat.charAt(0).toUpperCase() + cat.slice(1)\"></option></template></select></div><div><select x-model=\"severityFilter\" class=\"input text-sm\"><option value=\"\">All Severities</option> <option value=\"critical\">Critical</option> <option value=\"high\">High</option> <option value=\"medium\">Medium</option> <option value=\"low\">Low</option></select></div><span class=\"text-sm text-gray-400\" x-text=\"filteredPolicies().length + ' policies'\"></span></div><!-- Policies Table --><div x-show=\"!loading && !error && policies.length > 0\" x-cloak class=\"card overflow-hidden\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Policy</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Category</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Severity</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Status</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Enforcement</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Actions</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"policy in filteredPolicies()\" :key=\"policy.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><div class=\"font-medium text-white\" x-text=\"policy.name\"></div><div class=\"text-sm text-gray-400 mt-0.5 max-w-[300px] truncate\" x-text=\"policy.description\"></div></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"categoryClass(policy.category)\" x-text=\"policy.category\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"severityClass(policy.severity)\" x-text=\"policy.severity\"></span></td><td class=\"px-4 py-3\"><template x-if=\"policy.is_enabled\"><span class=\"px-2 py-0.5 text-xs bg-green-500/20 text-green-400 rounded flex items-center gap-1 w-fit\"><span class=\"w-1.5 h-1.5 rounded-full bg-green-400\"></span>Enabled</span></template><template x-if=\"!policy.is_enabled\"><span class=\"px-2 py-0.5 text-xs bg-gray-500/20 text-gray-400 rounded flex items-center gap-1 w-fit\"><span class=\"w-1.5 h-1.5 rounded-full bg-gray-400\"></span>Disabled</span></template></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-300\" x-text=\"policy.is_enforcing ? 'Enforcing' : 'Warn (Audit Only)'\"></span></td><td class=\"px-4 py-3\"><button @click=\"openEvaluateModal(policy)\" class=\"p-1.5 text-gray-400 hover:text-primary-400 transition-colors\" title=\"Evaluate against container\"><i aria-hidden=\"true\" class=\"fas fa-play-circle text-sm\"></i></button></td></tr></template></tbody></table></div><!-- Empty State --><div x-show=\"!loading && !error && policies.length === 0\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">No OPA Policies</h3><p class=\"text-gray-400 mb-4\">Seed default policies or create custom ones to enforce container compliance</p><button @click=\"seedPolicies()\" :disabled=\"seeding\" class=\"btn-primary\"><i aria-hidden=\"true\" class=\"fas fa-database mr-2\"></i>Seed Default Policies</button></div><!-- Evaluate Modal --><div x-show=\"showEvaluateModal\" x-cloak class=\"fixed inset-0 z-50 flex items-center justify-center bg-black/60\"><div class=\"bg-dark-800 rounded-xl shadow-xl border border-dark-600 w-full max-w-md mx-4\"><div class=\"flex items-center justify-between p-6 border-b border-dark-600\"><h2 class=\"text-lg font-display font-bold text-white\">Evaluate Policy</h2><button @click=\"showEvaluateModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-6 space-y-4\"><div x-show=\"selectedPolicy\"><p class=\"text-sm text-gray-400 mb-1\">Policy</p><p class=\"text-white font-medium\" x-text=\"selectedPolicy?.name\"></p></div><div><label class=\"block text-sm font-medium text-gray-300 mb-1\">Container ID</label> <input type=\"text\" x-model=\"evaluateContainerID\" class=\"input w-full font-mono\" placeholder=\"Enter container ID or name\" aria-label=\"Container ID\"></div><!-- Evaluation Results --><div x-show=\"evaluateResult\" x-cloak class=\"mt-4\"><h4 class=\"text-sm font-medium text-gray-300 mb-2\">Results</h4><pre class=\"bg-dark-900 border border-dark-600 rounded-lg p-3 text-sm text-gray-300 font-mono overflow-x-auto max-h-48 overflow-y-auto whitespace-pre-wrap\" x-text=\"evaluateResult ? JSON.stringify(evaluateResult, null, 2) : ''\"></pre></div><div class=\"flex justify-end gap-3 pt-4 border-t border-dark-600\"><button @click=\"showEvaluateModal = false\" class=\"btn-secondary\">Cancel</button> <button @click=\"evaluatePolicy()\" :disabled=\"evaluating || !evaluateContainerID\" class=\"btn-primary\"><i aria-hidden=\"true\" class=\"fas fa-play mr-2\" :class=\"evaluating && 'animate-spin'\"></i> <span x-text=\"evaluating ? 'Evaluating...' : 'Evaluate'\"></span></button></div></div></div></div></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
