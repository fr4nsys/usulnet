...
func ImageSigning(data ImageSigningData) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "\" class=\"space-y-6\"><!-- Header --><div class=\"flex items-center justify-between\"><div><h1 class=\"text-2xl font-display font-bold text-white\">Image Signing & Verification</h1><p class=\"text-gray-400 mt-1\">Verify image signatures and manage trust policies</p></div></div><!-- Tabs --><div class=\"space-y-4\"><div class=\"flex gap-1 border-b border-dark-600\"><button @click=\"tab = 'verify'\" :class=\"tab === 'verify' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"tab-underline\"><i aria-hidden=\"true\" class=\"fas fa-check-double mr-2\"></i>Verify Image</button> <button @click=\"tab = 'policies'; loadPolicies()\" :class=\"tab === 'policies' ? 'border-primary-500 text-white' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"tab-underline\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt mr-2\"></i>Trust Policies</button></div><!-- Verify Image Tab --><div x-show=\"tab === 'verify'\" x-cloak><div class=\"card p-6 space-y-6\"><!-- Input --><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Image Reference</label><div class=\"flex gap-3\"><input type=\"text\" x-model=\"imageRef\" class=\"input flex-1 font-mono\" placeholder=\"e.g. nginx:latest, registry.example.com/app:v1.2.3\" @keydown.enter.prevent=\"checkSignatures()\" aria-label=\"Image Reference\"> <button @click=\"checkSignatures()\" :disabled=\"!imageRef.trim() || sigLoading\" class=\"btn-primary flex items-center gap-2 whitespace-nowrap disabled:opacity-50\"><i aria-hidden=\"true\" class=\"fas fa-file-signature\" :class=\"sigLoading && 'animate-pulse'\"></i> Check Signatures</button> <button @click=\"verifyPolicies()\" :disabled=\"!imageRef.trim() || verifyLoading\" class=\"btn-secondary flex items-center gap-2 whitespace-nowrap disabled:opacity-50\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt\" :class=\"verifyLoading && 'animate-pulse'\"></i> Verify Against Policies</button></div></div><!-- Signatures Results --><div x-show=\"signatures !== null\" x-cloak class=\"space-y-4\"><h3 class=\"text-lg font-medium text-white\"><i aria-hidden=\"true\" class=\"fas fa-file-signature mr-2 text-primary-400\"></i>Signatures</h3><template x-if=\"signatures && signatures.length === 0\"><div class=\"bg-dark-700 rounded-lg p-6 text-center\"><i aria-hidden=\"true\" class=\"fas fa-times-circle text-3xl text-yellow-400 mb-3\"></i><p class=\"text-gray-300\">No signatures found for this image</p><p class=\"text-sm text-gray-500 mt-1\">This image has not been signed or signatures are not accessible</p></div></template><template x-if=\"signatures && signatures.length > 0\"><div class=\"bg-dark-800 border border-dark-700 rounded-xl overflow-hidden\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Issuer</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Transparency Log ID</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Created At</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Signature Type</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"sig in signatures\" :key=\"sig.transparency_log_id || sig.issuer\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><span class=\"text-sm text-white\" x-text=\"sig.issuer || 'Unknown'\"></span></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-300 font-mono\" x-text=\"sig.transparency_log_id || '-'\"></span></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-400\" x-text=\"sig.created_at || '-'\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs bg-dark-600 text-gray-300 rounded\" x-text=\"sig.signature_type || 'N/A'\"></span></td></tr></template></tbody></table></div></template></div><!-- Verification Results --><div x-show=\"verification !== null\" x-cloak class=\"space-y-4\"><h3 class=\"text-lg font-medium text-white\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt mr-2 text-primary-400\"></i>Policy Verification</h3><!-- Overall Status --><div :class=\"verification && verification.allowed ? 'border-green-500/30 bg-green-500/5' : 'border-red-500/30 bg-red-500/5'\" class=\"border rounded-lg p-4 flex items-center gap-4\"><div :class=\"verification && verification.allowed ? 'bg-green-500/20' : 'bg-red-500/20'\" class=\"w-12 h-12 rounded-full flex items-center justify-center\"><i :class=\"verification && verification.allowed ? 'fa-check-circle text-green-400' : 'fa-times-circle text-red-400'\" class=\"fas text-xl\"></i></div><div><h4 :class=\"verification && verification.allowed ? 'text-green-400' : 'text-red-400'\" class=\"font-medium\" x-text=\"verification && verification.allowed ? 'Verification Passed' : 'Verification Failed'\"></h4><p class=\"text-sm text-gray-400\" x-text=\"verification && verification.matched_policies && verification.matched_policies.length > 0 ? 'Matched policies: ' + verification.matched_policies.join(', ') : 'No matching policies'\"></p></div></div><!-- Violations --><template x-if=\"verification && verification.violations && verification.violations.length > 0\"><div class=\"space-y-2\"><h4 class=\"text-sm font-medium text-red-400 flex items-center gap-2\"><i aria-hidden=\"true\" class=\"fas fa-times-circle\"></i>Violations</h4><template x-for=\"(violation, idx) in verification.violations\" :key=\"'v-' + idx\"><div class=\"bg-dark-700 rounded-lg p-3 flex items-center gap-3\"><i aria-hidden=\"true\" class=\"fas fa-times text-red-400\"></i> <span class=\"text-sm text-gray-300\" x-text=\"violation\"></span></div></template></div></template><!-- Warnings --><template x-if=\"verification && verification.warnings && verification.warnings.length > 0\"><div class=\"space-y-2\"><h4 class=\"text-sm font-medium text-yellow-400 flex items-center gap-2\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-triangle\"></i>Warnings</h4><template x-for=\"(warning, idx) in verification.warnings\" :key=\"'w-' + idx\"><div class=\"bg-dark-700 rounded-lg p-3 flex items-center gap-3\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-triangle text-yellow-400\"></i> <span class=\"text-sm text-gray-300\" x-text=\"warning\"></span></div></template></div></template></div><!-- Error --><div x-show=\"error\" x-cloak class=\"bg-red-500/10 border border-red-500/20 rounded-lg p-4 flex items-center gap-3\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-circle text-red-400\"></i> <span class=\"text-red-400 text-sm\" x-text=\"error\"></span></div></div></div><!-- Trust Policies Tab --><div x-show=\"tab === 'policies'\" x-cloak class=\"space-y-4\"><!-- Actions --><div class=\"flex justify-end\"><button @click=\"seedPolicies()\" :disabled=\"seedLoading\" class=\"btn-secondary flex items-center gap-2 disabled:opacity-50\"><i aria-hidden=\"true\" class=\"fas fa-seedling\" :class=\"seedLoading && 'animate-pulse'\"></i> Seed Default Policies</button></div><!-- Loading --><div x-show=\"policiesLoading\" class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl text-gray-400 mb-3\"></i><p class=\"text-gray-400\">Loading trust policies...</p></div><!-- Empty State --><div x-show=\"!policiesLoading && policies.length === 0\" x-cloak class=\"card p-12 text-center\"><i aria-hidden=\"true\" class=\"fas fa-shield-alt text-4xl text-gray-400 mb-4\"></i><h3 class=\"text-lg font-medium text-white mb-2\">No Trust Policies</h3><p class=\"text-gray-400 mb-4\">Trust policies define which signers are required for image verification</p><button @click=\"seedPolicies()\" :disabled=\"seedLoading\" class=\"btn-primary inline-flex items-center gap-2 disabled:opacity-50\"><i aria-hidden=\"true\" class=\"fas fa-seedling\"></i> Seed Default Policies</button></div><!-- Policies Table --><div x-show=\"!policiesLoading && policies.length > 0\" x-cloak class=\"bg-dark-800 border border-dark-700 rounded-xl overflow-hidden\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600\"><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Policy Name</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Description</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Allowed Signers</th><th scope=\"col\" class=\"text-left text-xs font-medium text-gray-400 uppercase px-4 py-3\">Status</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"policy in policies\" :key=\"policy.name || policy.id\"><tr class=\"hover:bg-dark-700/50\"><td class=\"px-4 py-3\"><span class=\"text-sm font-medium text-white\" x-text=\"policy.name\"></span></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-400\" x-text=\"policy.description || '-'\"></span></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-300 font-mono\" x-text=\"policy.allowed_signers && policy.allowed_signers.length > 0 ? policy.allowed_signers.join(', ') : 'Any'\"></span></td><td class=\"px-4 py-3\"><template x-if=\"policy.is_enforcing\"><span class=\"px-2 py-0.5 text-xs bg-red-500/20 text-red-400 rounded flex items-center gap-1 w-fit\"><i aria-hidden=\"true\" class=\"fas fa-lock text-[10px]\"></i>Enforcing</span></template><template x-if=\"!policy.is_enforcing\"><span class=\"px-2 py-0.5 text-xs bg-gray-500/20 text-gray-400 rounded\">Audit Only</span></template></td></tr></template></tbody></table></div></div></div></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
