...
func GitSync(data GitSyncData) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Var2 := templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "\" x-init=\"init()\" class=\"space-y-6\" x-cloak><!-- Header --><div class=\"flex items-center justify-between\"><div><h1 class=\"text-2xl font-display font-bold text-white\">Git Sync</h1><p class=\"text-gray-400 mt-1\">Bidirectional synchronization between Git repositories and UI-managed stacks</p></div><button @click=\"showCreateModal = true\" class=\"btn-primary\"><i aria-hidden=\"true\" class=\"fas fa-plus mr-2\"></i>New Sync Config</button></div><!-- Stats Cards --><div class=\"grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4\"><div class=\"card p-5\"><div class=\"flex items-center gap-3\"><div class=\"w-10 h-10 rounded-lg bg-primary-500/10 flex items-center justify-center\"><i aria-hidden=\"true\" class=\"fas fa-cogs text-primary-400\"></i></div><div><p class=\"text-2xl font-bold text-white\" x-text=\"stats.total_configs || 0\"></p><p class=\"text-xs text-gray-500\">Total Configs</p></div></div></div><div class=\"card p-5\"><div class=\"flex items-center gap-3\"><div class=\"w-10 h-10 rounded-lg bg-green-500/10 flex items-center justify-center\"><i aria-hidden=\"true\" class=\"fas fa-sync text-green-400\"></i></div><div><p class=\"text-2xl font-bold text-white\" x-text=\"stats.active_configs || 0\"></p><p class=\"text-xs text-gray-500\">Active Syncs</p></div></div></div><div class=\"card p-5\"><div class=\"flex items-center gap-3\"><div class=\"w-10 h-10 rounded-lg bg-blue-500/10 flex items-center justify-center\"><i aria-hidden=\"true\" class=\"fas fa-clock text-blue-400\"></i></div><div><p class=\"text-2xl font-bold text-white\" x-text=\"stats.total_syncs || 0\"></p><p class=\"text-xs text-gray-500\">Total Syncs</p></div></div></div><div class=\"card p-5\"><div class=\"flex items-center gap-3\"><div class=\"w-10 h-10 rounded-lg bg-yellow-500/10 flex items-center justify-center\"><i aria-hidden=\"true\" class=\"fas fa-exclamation-triangle text-yellow-400\"></i></div><div><p class=\"text-2xl font-bold text-white\" x-text=\"stats.pending_conflicts || 0\"></p><p class=\"text-xs text-gray-500\">Pending Conflicts</p></div></div></div></div><!-- Tabs --><div class=\"border-b border-dark-600\"><nav class=\"flex gap-6\"><button @click=\"activeTab = 'configs'\" :class=\"activeTab === 'configs' ? 'border-primary-500 text-primary-400' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"pb-3 border-b-2 text-sm font-medium transition-colors\"><i aria-hidden=\"true\" class=\"fas fa-cogs mr-2\"></i>Configurations</button> <button @click=\"activeTab = 'events'; loadEvents()\" :class=\"activeTab === 'events' ? 'border-primary-500 text-primary-400' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"pb-3 border-b-2 text-sm font-medium transition-colors\"><i aria-hidden=\"true\" class=\"fas fa-stream mr-2\"></i>Sync Events</button> <button @click=\"activeTab = 'conflicts'; loadConflicts()\" :class=\"activeTab === 'conflicts' ? 'border-primary-500 text-primary-400' : 'border-transparent text-gray-400 hover:text-gray-300'\" class=\"pb-3 border-b-2 text-sm font-medium transition-colors\"><i aria-hidden=\"true\" class=\"fas fa-code-branch mr-2\"></i>Conflicts <span x-show=\"stats.pending_conflicts > 0\" class=\"ml-1 px-1.5 py-0.5 text-xs rounded-full bg-yellow-500/20 text-yellow-400\" x-text=\"stats.pending_conflicts\"></span></button></nav></div><!-- Configurations Tab --><div x-show=\"activeTab === 'configs'\"><div class=\"card overflow-hidden\"><template x-if=\"loading\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl mb-3\"></i><p>Loading configurations...</p></div></template><template x-if=\"!loading && configs.length === 0\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-code-branch text-3xl mb-3\"></i><p>No sync configurations yet</p><p class=\"text-sm mt-1\">Create one to start syncing between Git and your stacks.</p></div></template><template x-if=\"!loading && configs.length > 0\"><div class=\"overflow-x-auto\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600 text-left\"><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Name</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Branch</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Direction</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Auto-Sync</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Last Sync</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Status</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Actions</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"cfg in configs\" :key=\"cfg.id\"><tr class=\"hover:bg-dark-700/50 transition-colors\"><td class=\"px-4 py-3\"><div class=\"font-medium text-white\" x-text=\"cfg.name\"></div><div class=\"text-xs text-gray-500 truncate max-w-xs\" x-text=\"cfg.stack_name\"></div></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-300 font-mono\" x-text=\"cfg.branch\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-1 text-xs rounded font-medium\" :class=\"directionBadge(cfg.sync_direction)\" x-text=\"directionLabel(cfg.sync_direction)\"></span></td><td class=\"px-4 py-3\"><button @click=\"toggleAutoSync(cfg)\" class=\"relative inline-flex h-5 w-10 items-center rounded-full transition-colors\" :class=\"cfg.is_enabled ? 'bg-primary-500' : 'bg-dark-500'\"><span class=\"inline-block h-4 w-4 transform rounded-full bg-white transition-transform\" :class=\"cfg.is_enabled ? 'translate-x-5' : 'translate-x-1'\"></span></button></td><td class=\"px-4 py-3\"><span class=\"text-sm text-gray-400\" x-text=\"cfg.last_sync_at ? new Date(cfg.last_sync_at).toLocaleString() : 'Never'\"></span></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"statusBadge(cfg.last_sync_status)\" x-text=\"cfg.last_sync_status || 'idle'\"></span></td><td class=\"px-4 py-3\"><div class=\"flex items-center gap-2\"><button @click=\"triggerSync(cfg)\" class=\"p-1.5 text-gray-400 hover:text-green-400 transition-colors\" title=\"Trigger Sync Now\" :disabled=\"!cfg.is_enabled || cfg.syncing\"><i aria-hidden=\"true\" class=\"fas fa-sync text-sm\" :class=\"cfg.syncing ? 'fa-spin' : ''\"></i></button> <button @click=\"viewEvents(cfg)\" class=\"p-1.5 text-gray-400 hover:text-blue-400 transition-colors\" title=\"View Events\"><i aria-hidden=\"true\" class=\"fas fa-stream text-sm\"></i></button> <button @click=\"deleteConfig(cfg)\" class=\"p-1.5 text-gray-400 hover:text-red-400 transition-colors\" title=\"Delete\"><i aria-hidden=\"true\" class=\"fas fa-trash text-sm\"></i></button></div></td></tr></template></tbody></table></div></template></div></div><!-- Events Tab --><div x-show=\"activeTab === 'events'\"><!-- Config filter for events --><div class=\"mb-4\"><select @change=\"selectedConfigId = $event.target.value; loadEvents()\" class=\"input w-64\"><option value=\"\">All Configurations</option><template x-for=\"cfg in configs\" :key=\"cfg.id\"><option :value=\"cfg.id\" x-text=\"cfg.name\"></option></template></select></div><div class=\"card overflow-hidden\"><template x-if=\"eventsLoading\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl mb-3\"></i><p>Loading events...</p></div></template><template x-if=\"!eventsLoading && events.length === 0\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-stream text-3xl mb-3\"></i><p>No sync events recorded yet</p></div></template><template x-if=\"!eventsLoading && events.length > 0\"><div class=\"overflow-x-auto\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600 text-left\"><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Time</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Config</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Direction</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Type</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Status</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Message</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"evt in events\" :key=\"evt.id\"><tr class=\"hover:bg-dark-700/50 transition-colors\"><td class=\"px-4 py-3 text-sm text-gray-400 whitespace-nowrap\" x-text=\"new Date(evt.created_at).toLocaleString()\"></td><td class=\"px-4 py-3 text-sm text-white\" x-text=\"configName(evt.config_id)\"></td><td class=\"px-4 py-3\"><span class=\"px-2 py-1 text-xs rounded font-medium\" :class=\"directionBadge(evt.direction)\" x-text=\"directionLabel(evt.direction)\"></span></td><td class=\"px-4 py-3 text-sm text-gray-300\" x-text=\"evt.event_type\"></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"statusBadge(evt.status)\" x-text=\"evt.status\"></span></td><td class=\"px-4 py-3 text-sm text-gray-400 max-w-xs truncate\"><span x-text=\"evt.commit_message || evt.error_message || '-'\"></span></td></tr></template></tbody></table></div></template></div></div><!-- Conflicts Tab --><div x-show=\"activeTab === 'conflicts'\"><div class=\"card overflow-hidden\"><template x-if=\"conflictsLoading\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl mb-3\"></i><p>Loading conflicts...</p></div></template><template x-if=\"!conflictsLoading && conflicts.length === 0\"><div class=\"p-8 text-center text-gray-400\"><i aria-hidden=\"true\" class=\"fas fa-check-circle text-3xl mb-3 text-green-500\"></i><p>No unresolved conflicts</p></div></template><template x-if=\"!conflictsLoading && conflicts.length > 0\"><div class=\"overflow-x-auto\"><table class=\"w-full\"><thead><tr class=\"border-b border-dark-600 text-left\"><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Config</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">File Path</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Detected</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Status</th><th scope=\"col\" class=\"px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider\">Actions</th></tr></thead> <tbody class=\"divide-y divide-dark-700\"><template x-for=\"conflict in conflicts\" :key=\"conflict.id\"><tr class=\"hover:bg-dark-700/50 transition-colors\"><td class=\"px-4 py-3 text-sm text-white\" x-text=\"configName(conflict.config_id)\"></td><td class=\"px-4 py-3 text-sm text-gray-300 font-mono\" x-text=\"conflict.file_path\"></td><td class=\"px-4 py-3 text-sm text-gray-400 whitespace-nowrap\" x-text=\"new Date(conflict.created_at).toLocaleString()\"></td><td class=\"px-4 py-3\"><span class=\"px-2 py-0.5 text-xs rounded\" :class=\"conflict.resolution === 'pending' ? 'bg-yellow-500/20 text-yellow-400' : 'bg-green-500/20 text-green-400'\" x-text=\"conflict.resolution\"></span></td><td class=\"px-4 py-3\"><template x-if=\"conflict.resolution === 'pending'\"><div class=\"flex items-center gap-2\"><button @click=\"resolveConflict(conflict, 'use_git')\" class=\"px-2 py-1 text-xs rounded bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 transition-colors\" title=\"Use content from Git\">Use Git</button> <button @click=\"resolveConflict(conflict, 'use_ui')\" class=\"px-2 py-1 text-xs rounded bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 transition-colors\" title=\"Use content from UI\">Use UI</button> <button @click=\"openManualMerge(conflict)\" class=\"px-2 py-1 text-xs rounded bg-yellow-500/20 text-yellow-400 hover:bg-yellow-500/30 transition-colors\" title=\"Manually merge content\">Manual</button></div></template><template x-if=\"conflict.resolution !== 'pending'\"><span class=\"text-sm text-gray-500\">Resolved</span></template></td></tr></template></tbody></table></div></template></div></div><!-- Create Config Modal --><template x-if=\"showCreateModal\"><div class=\"fixed inset-0 z-50 flex items-center justify-center p-4\" @keydown.escape.window=\"showCreateModal = false\"><div class=\"fixed inset-0 bg-black/60\" @click=\"showCreateModal = false\"></div><div class=\"relative card w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl\"><div class=\"flex items-center justify-between p-5 border-b border-dark-600\"><h3 class=\"text-lg font-semibold text-white\">New Sync Configuration</h3><button @click=\"showCreateModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-5 space-y-4\"><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Name</label> <input type=\"text\" x-model=\"newConfig.name\" class=\"input w-full\" placeholder=\"my-stack-sync\" aria-label=\"my-stack-sync\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Connection ID</label> <input type=\"text\" x-model=\"newConfig.connection_id\" class=\"input w-full font-mono\" placeholder=\"UUID of the Git connection\" aria-label=\"UUID of the Git connection\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Repository ID</label> <input type=\"text\" x-model=\"newConfig.repository_id\" class=\"input w-full font-mono\" placeholder=\"UUID of the Git repository\" aria-label=\"UUID of the Git repository\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Repository Full Name</label> <input type=\"text\" x-model=\"newConfig.repo_full_name\" class=\"input w-full font-mono\" placeholder=\"org/repo\" aria-label=\"org/repo\"><p class=\"text-xs text-gray-500 mt-1\">Full name of the Git repository (e.g. owner/repo)</p></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Stack Name</label> <input type=\"text\" x-model=\"newConfig.stack_name\" class=\"input w-full\" placeholder=\"my-stack\" aria-label=\"my-stack\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Target Path</label> <input type=\"text\" x-model=\"newConfig.target_path\" class=\"input w-full font-mono\" placeholder=\"stacks/my-stack\" aria-label=\"stacks/my-stack\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Branch</label> <input type=\"text\" x-model=\"newConfig.branch\" class=\"input w-full font-mono\" placeholder=\"main\" aria-label=\"main\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">File Pattern</label> <input type=\"text\" x-model=\"newConfig.file_pattern\" class=\"input w-full font-mono\" placeholder=\"docker-compose*.yml\" aria-label=\"docker-compose*.yml\"></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Sync Direction</label> <select x-model=\"newConfig.sync_direction\" class=\"input w-full\"><option value=\"from_git\">Git → UI</option> <option value=\"to_git\">UI → Git</option> <option value=\"bidirectional\">Bidirectional</option></select></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Conflict Strategy</label> <select x-model=\"newConfig.conflict_strategy\" class=\"input w-full\"><option value=\"manual\">Manual Resolution</option> <option value=\"prefer_git\">Prefer Git</option> <option value=\"prefer_ui\">Prefer UI</option></select></div><div class=\"flex items-center gap-4\"><label class=\"flex items-center gap-2 cursor-pointer\"><input type=\"checkbox\" x-model=\"newConfig.auto_commit\" class=\"rounded border-dark-600 bg-dark-700 text-primary-500\"> <span class=\"text-sm text-gray-300\">Auto-commit</span></label> <label class=\"flex items-center gap-2 cursor-pointer\"><input type=\"checkbox\" x-model=\"newConfig.auto_deploy\" class=\"rounded border-dark-600 bg-dark-700 text-primary-500\"> <span class=\"text-sm text-gray-300\">Auto-deploy</span></label></div></div><div class=\"flex justify-end gap-3 p-5 border-t border-dark-600\"><button @click=\"showCreateModal = false\" class=\"btn-secondary\">Cancel</button> <button @click=\"createConfig()\" class=\"btn-primary\" :disabled=\"!newConfig.name || creating\"><i aria-hidden=\"true\" class=\"fas fa-plus mr-2\" x-show=\"!creating\"></i> <i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin mr-2\" x-show=\"creating\"></i> <span x-text=\"creating ? 'Creating...' : 'Create'\"></span></button></div></div></div></template><!-- Manual Merge Modal --><template x-if=\"showMergeModal\"><div class=\"fixed inset-0 z-50 flex items-center justify-center p-4\" @keydown.escape.window=\"showMergeModal = false\"><div class=\"fixed inset-0 bg-black/60\" @click=\"showMergeModal = false\"></div><div class=\"relative card w-full max-w-3xl max-h-[90vh] overflow-y-auto shadow-2xl\"><div class=\"flex items-center justify-between p-5 border-b border-dark-600\"><h3 class=\"text-lg font-semibold text-white\">Manual Merge: <span class=\"text-gray-400 font-mono text-sm\" x-text=\"mergeConflict?.file_path\"></span></h3><button @click=\"showMergeModal = false\" class=\"text-gray-400 hover:text-white\"><i aria-hidden=\"true\" class=\"fas fa-times\"></i></button></div><div class=\"p-5 space-y-4\"><div class=\"grid grid-cols-2 gap-4\"><div><label class=\"block text-sm font-medium text-gray-300 mb-2\"><i aria-hidden=\"true\" class=\"fas fa-code-branch mr-1 text-blue-400\"></i>Git Content</label><pre class=\"bg-dark-900 border border-dark-600 rounded-lg p-3 text-sm text-gray-300 font-mono overflow-auto max-h-48 whitespace-pre-wrap\" x-text=\"mergeConflict?.git_content\"></pre></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\"><i aria-hidden=\"true\" class=\"fas fa-desktop mr-1 text-purple-400\"></i>UI Content</label><pre class=\"bg-dark-900 border border-dark-600 rounded-lg p-3 text-sm text-gray-300 font-mono overflow-auto max-h-48 whitespace-pre-wrap\" x-text=\"mergeConflict?.ui_content\"></pre></div></div><div><label class=\"block text-sm font-medium text-gray-300 mb-2\">Resolved Content</label> <textarea x-model=\"mergeContent\" class=\"input w-full font-mono h-48 resize-y\" placeholder=\"Enter the resolved content here...\"></textarea></div></div><div class=\"flex justify-end gap-3 p-5 border-t border-dark-600\"><button @click=\"showMergeModal = false\" class=\"btn-secondary\">Cancel</button> <button @click=\"submitManualMerge()\" class=\"btn-primary\" :disabled=\"!mergeContent\"><i aria-hidden=\"true\" class=\"fas fa-check mr-2\"></i>Apply Merge</button></div></div></div></template></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
			return nil
