...
func TerminalTabs(data TerminalTabsData) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 2, "\" class=\"flex items-center gap-2 px-3 py-2 text-gray-400 hover:text-white transition-colors\"><i aria-hidden=\"true\" class=\"fas fa-arrow-left text-sm\"></i> <span class=\"text-sm\">")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		...
		if data.AllowNewTab {
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 20, "<button @click=\"openNewTabModal()\" class=\"flex items-center justify-center w-8 h-8 text-gray-400 hover:text-white hover:bg-dark-700 rounded-lg transition-colors ml-1\" title=\"New Tab\"><i aria-hidden=\"true\" class=\"fas fa-plus text-sm\"></i></button>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
		templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 21, "<!-- Global Actions --><div class=\"flex items-center gap-1 ml-2\"><button @click=\"reconnectActive()\" class=\"p-2 text-gray-400 hover:text-white hover:bg-dark-700 rounded-lg transition-colors\" title=\"Reconnect\"><i aria-hidden=\"true\" class=\"fas fa-redo text-sm\"></i></button> <button @click=\"toggleFullscreen()\" class=\"p-2 text-gray-400 hover:text-white hover:bg-dark-700 rounded-lg transition-colors\" title=\"Fullscreen\"><i aria-hidden=\"true\" class=\"fas fa-expand text-sm\"></i></button></div></div><!-- Tab Status Bar --><div class=\"flex items-center justify-between px-4 py-1 bg-dark-850 border-b border-dark-700 text-xs\"><div class=\"flex items-center gap-4\"><span class=\"text-gray-500\">Tab: <span class=\"text-gray-300\" x-text=\"getActiveTabName()\"></span></span> <span x-show=\"getActiveStatus() === 'connected'\" class=\"flex items-center gap-1 text-green-400\"><span class=\"w-1.5 h-1.5 bg-green-500 rounded-full\"></span> Connected</span> <span x-show=\"getActiveStatus() === 'connecting'\" class=\"flex items-center gap-1 text-yellow-400\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin\"></i> Connecting...</span> <span x-show=\"getActiveStatus() === 'disconnected'\" class=\"flex items-center gap-1 text-red-400\"><span class=\"w-1.5 h-1.5 bg-red-500 rounded-full\"></span> Disconnected</span></div><div class=\"text-gray-500\"><span x-text=\"tabs.size\"></span> tab(s)</div></div><!-- Terminal Containers — visibility managed by switchTab() via style.display,\n\t\t     NOT Alpine x-show, to avoid conflicts with dynamically added tabs. --><div class=\"flex-1 relative bg-black\">")
		if templ_7745c5c3_Err != nil {
			return templ_7745c5c3_Err
		}
		for i, tab := range data.Tabs {
		...
		if data.AllowNewTab && data.NewTabURL != "" {
			...
			templ_7745c5c3_Err = templruntime.WriteString(templ_7745c5c3_Buffer, 30, "\" hx-trigger=\"intersect once\" hx-target=\"#new-tab-content\"><div id=\"new-tab-content\" class=\"space-y-4\"><div class=\"flex items-center justify-center py-8\"><i aria-hidden=\"true\" class=\"fas fa-spinner fa-spin text-2xl text-gray-500\"></i></div></div></div><div class=\"flex justify-end gap-3 mt-6\"><button @click=\"newTabModalOpen = false\" class=\"px-4 py-2 bg-dark-700 hover:bg-dark-600 text-gray-300 rounded-lg transition-colors\">Cancel</button></div></div></div>")
			if templ_7745c5c3_Err != nil {
				return templ_7745c5c3_Err
			}
		}
